...
# Размер подобран по факту, а не по умолчанию. При PERCENT=50 zram забился
# полностью (478 из 480 МБ) за первые же минуты, и вытеснение снова пошло на
# диск. При этом замер сжатия дал 4.06x: 480 МБ страниц занимали 118 МБ памяти —
# у JVM много однообразных и нулевых страниц, они жмутся прекрасно.
#
# 75% => ~720 МБ сжатого свопа ценой ~180 МБ физической памяти. Сотню процентов
# не берём: если сжимаемость упадёт, zram начнёт вытеснять сам себя.
cat > /etc/default/zramswap <<'EOF'
# Алгоритм: zstd жмёт заметно лучше lzo при сопоставимой скорости.
ALGO=zstd
# Три четверти физической памяти под сжатый своп — см. замер выше.
PERCENT=75
